    ...
    <header className="sticky top-0 z-40 glass border-b border-border/60">
      <div className="container flex h-14 items-center gap-4">
        ...
        <div className="flex items-center gap-1">
          <Link
            href="/create"
            className="hidden sm:inline-flex items-center gap-2 h-9 px-4 rounded-full bg-foreground text-background text-sm font-medium hover:opacity-90 transition press"
          >
            <PenSquare className="h-4 w-4" /> Post
          </Link>
          <Link href="/saved" className="hidden sm:inline-flex h-9 w-9 items-center justify-center rounded-full hover:bg-muted transition press" aria-label="Saved">
...
function MobileNav({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} className="flex flex-col items-center justify-center text-[10px] font-medium text-muted-foreground hover:text-foreground gap-0.5 px-3 py-1">
      {icon}
      {label}
    </Link>
  );
